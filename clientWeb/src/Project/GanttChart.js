        ...
        <Container fluid className="mb-3">
          <Row>
            ...
            <Col>
              <ToggleButtonGroup
                name="durationType"
                defaultValue={this.state.durationType}
              >
                <ToggleButton
                  variant="outline-secondary"
                  disabled
                ...
                <ToggleButton
                  variant="secondary"
                  value="week"
                  onClick={() => this.setState({ durationType: "week" })}
                >
                  Week
                </ToggleButton>
                <ToggleButton
                  variant="secondary"
                  value="month"
                  onClick={() => this.setState({ durationType: "month" })}
                >
                  Month
                </ToggleButton>
                <ToggleButton
                  variant="secondary"
                  value="threeMonth"
                  onClick={() => this.setState({ durationType: "threeMonth" })}
                >
                  Three Months
                </ToggleButton>
                <ToggleButton
                  variant="secondary"
                  value="sixMonth"
                  onClick={() => this.setState({ durationType: "sixMonth" })}
                >
                  Six Months
                </ToggleButton>
                <ToggleButton
                  variant="secondary"
                  value="all"
                  onClick={() => this.setState({ durationType: "all" })}
                >
                  All Tasks
                </ToggleButton>
              </ToggleButtonGroup>
...
class GanttChart extends React.Component {
  ...
  filterTasks() {
    ...
    for (var x = 0; x < tasks.length; x++) {
      startDate = new Date(tasks[x].startDate);
      if (
        startDate.valueOf() <= filterDate.valueOf() &&
        startDate.valueOf() >= beginFilterDate.valueOf()
      ) {
