...
export default class ScheduleTrackingComponent extends React.Component {
  formatData(tasks) {
    ...
    tasks.forEach((task) => {
      if (new Date(task.endDate).valueOf() < new Date().valueOf())
        expectedPercentage = 100;
      else {
        expectedPercentage =
          ((new Date().valueOf() - new Date(task.startDate).valueOf()) /
            task.duration) *
          100;
      }

      data.push([task.name, task.progress, Math.round(expectedPercentage)]);
    });
  ...
  filterTasks(tasks) {
    tasks = tasks.filter((task) => {
      return (
        (task.type !== "Complete" &&
          new Date(task.endDate).valueOf() < new Date().valueOf()) ||
        (new Date(task.startDate).valueOf() < new Date().valueOf() &&
          new Date(task.endDate).valueOf() > new Date().valueOf())
      );
    });
