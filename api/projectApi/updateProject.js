...
function addNewQuery(currentNode, startDate, queriesArray){
    let endDate = addDays(startDate[0], startDate[1], startDate[2], currentNode.duration)
    ...
    let query = 'MATCH(n) WHERE ID(n) = '+currentNode.id+' SET n +={startDate:date("'+startDate[0]+'-'+startDate[1]+'-'+startDate[2]+'"), endDate:date("'+endDate[0]+'-'+endDate[1]+'-'+endDate[2]+'")}'
